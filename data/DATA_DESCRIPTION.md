...
The same data is browsable interactively via the [UCSC Table Browser](https://genome.ucsc.edu/cgi-bin/hgTables) by selecting assembly = hg38, group = Repeats, track = Simple Repeats, table = simpleRepeat. For reproducibility, the raw table was downloaded directly from UCSC's hgdownload server (the same data source the Table Browser reads from) rather than exported via the web form.
